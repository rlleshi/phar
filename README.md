...
`python tools/demo/multimodial_demo.py video.mp4 demo.json --timestamps`

### Tag Generator

Given the predictions generated by the multimodial demo (in json), we can grab the top tags like so:
...
Best performing models (performance & runtime wise) are `timesformer` for the RGB stream, `poseC3D` for the skeleton stream and `resnet101` for the Audio stream. The results of these models are fused together, based on fine-tuned weights about their importance, through late-fusion.
...
A HAR model trained on 3D poses might be able solve this camera-perspective problem. However, due to the fact that 3D pose estimation is less accurate than 2D pose estimation, and I already noticed problems with the accuracy of the latter, this was not tried (yet). Ideally, however, if the dataset is big enough then the camera perspective problem should be naturally solved.
...
Due to the variety of positions and camera angles, which make the pose estimation difficult, it's only feasible to apply HAR on skeleton data on a few of the actions. The clips generated for the RGB dataset were filtered based on two criteria:
